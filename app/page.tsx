  ...
  async function sendChat() {
    ...
    try {
      ...
      if (j.updated?.metrics) {
        ...
        // também atualiza a leitura em memória com as métricas recalculadas
  ...
  return (
    <div style={{ maxWidth: 980, margin: '24px auto', padding: 12 }}>
      ...
      <label style={{ display:'block', marginTop: 12 }}>Como você tem se sentido nos últimos dias/semanas?
        <textarea rows={3} value={feel} onChange={e=>setFeel(e.target.value)} placeholder="Ex.: ansioso com prazos; dormindo mal; animado com projeto pessoal" style={{...inp, resize:'vertical'}} />
      </label>
      ...
      {reading && (
        <div style={{ marginTop: 20 }}>
          <div style={{ display:'grid', gap:12, gridTemplateColumns:'1fr 1fr' }}>
            ...
            <Card title="Tríptico" body={`ELI5: ${reading.block_E_triptych?.eli5||''}\n\nScientist: ${reading.block_E_triptych?.scientist||''}\n\nPoet: ${reading.block_E_triptych?.poet||''}`} full />
          </div>
          ...
          <h3>Diagnóstico</h3>
          <pre style={pre}>{(reading.diagnostic?.summary||'') + '\n\nAções: ' + ((reading.diagnostic?.actions||[]).join('; '))}</pre>
          ...
          <div style={chatbox}>
            ...
            {!history.length && <div style={{ opacity:.6 }}>Ex.: "Hoje estou ansioso por causa de X… o que ajustar?"</div>}
          </div>
...
1. Adicione estes dois arquivos **nesta estrutura**: `app/layout.tsx` e `app/page.tsx`.
2. Garanta que o projeto tem `package.json` com Next/React (como já combinamos) e que suas rotas estão em `app/api/...`.
3. (Opcional) Renomeie `public/index.html` para `_index-legacy.html` para não confundir.
4. Commit & push. No Vercel, o build deve detectar **Framework: Next.js**. Abra o Preview → `/` já será esta página.
