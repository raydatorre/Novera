...
export default function Page() {
  ...
  const [feel, setFeel] = useState(''); // como você tem se sentido
  const [R, setR] = useState<string>(''); // manual opcional
  const [Q, setQ] = useState<string>(''); // manual opcional
  ...
  useEffect(() => { try { const s = localStorage.getItem('oraculo:last'); if (s) setReading(JSON.parse(s)); } catch {} }, []);
    ...
    try {
      const res = await fetch('/api/oraculo', {
        ...
        body: JSON.stringify({
          full_name: fullName,
          dob_DDMMYYYY: dob,
          feelings: feel,                 // <<< estimador de R/Q
          R: R.trim() ? Number(R) : null, // opcional manual
  ...
  return (
    <div className="container">
      ...
      <label className="sectionTitle">Como você tem se sentido nos últimos dias/semanas?</label>
      <textarea className="txt" rows={3} value={feel} onChange={e=>setFeel(e.target.value)} placeholder="Ex.: ansioso com prazos, dormindo pouco, mas animado com um projeto" />
...
Poet: {reading.block_E_triptych?.poet || ''}
          ...
          <div className="card" style={{ gridColumn:'1 / -1' }}>
            ...
            {Array.isArray(reading.diagnostic?.actions) && reading.diagnostic!.actions!.length > 0 && (
              <ul>{reading.diagnostic!.actions!.map((a,i)=><li key={i}>{a}</li>)}</ul>
            )}
          </div>
      ...
      {!!history.length && (
        <div className="card" style={{ marginTop: 12 }}>
          ...
          {recalc && <p style={{ color:'#3347B0', fontWeight:700 }}>
            Recalibrado agora → FPC {recalc.FPC} • φ° {recalc.phi_deg} • FP {recalc.FP}
          </p>}
        </div>
      )}
    </div>
